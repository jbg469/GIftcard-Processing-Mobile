...
In the same 
ProductScrollingActivity.kt and CardScrollingActivity we comment out the last remaining references to anything "sensor" related that we didn't get for part 4b. Getting rid of all sensor functions in these two files is enough to remove all unnecessary sensor interactions from the mobile application, leaving only those necessary for the application to function and serve its purpose.
